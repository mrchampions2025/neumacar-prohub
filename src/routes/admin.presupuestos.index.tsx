  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="font-display text-2xl font-bold uppercase text-slate-800">Solicitudes de Presupuestos</h1>
              ...
              {loading ? (
                ...
                filteredLeads.map((l) => (
                  <tr key={l.id} className="transition-colors hover:bg-slate-50">
                    ...
                    <td className="p-4">
                      <span className="font-medium capitalize text-slate-800">{l.data?.service || "General"}</span>
                      {l.reference && (
                        <div className="text-xs text-[#1da1f2] font-semibold mt-1">Ref: {l.reference}</div>
                      )}
                    </td>
                    <td className="p-4">
      ...
      <Dialog open={!!selectedLead && !isBuildingQuote} onOpenChange={(open) => !open && setSelectedLead(null)}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto bg-white text-slate-800 border-slate-200">
          <DialogHeader>
            ...
            <div className="space-y-6 mt-4">
              ...
              {selectedLead.message && (
                <div>
                  ...
                  <p className="text-sm bg-slate-50 p-3 rounded-md border border-slate-100">{selectedLead.message}</p>
                </div>
              )}
